// src/native/db.js — the Android twin of electron/db.cjs.
//
// This is a DIRECT PORT: every query, every rule, every comment below the
// header is the desktop file unchanged. Only the four things that cannot
// exist inside a WebView were replaced:
//   1. better-sqlite3            -> the sql.js shim in ./sqlite.js
//   2. app.getPath('userData')   -> a storage adapter (Capacitor Filesystem)
//   3. node:crypto sha256        -> Web Crypto SHA-256, same hex output, so
//                                   PINs set on the PC still unlock here
//   4. db.backup() / db.close()  -> export() + a flush to the storage adapter
// Keeping the logic identical is the whole point: the schema, the money
// maths and the streak maths are shared with the desktop app, and a
// database file copied from %APPDATA%\LifeOS opens here as-is.
import { openDatabase } from './sqlite.js'
import { localISO, parseISO, addDaysISO, daysBetween, monthStartISO } from '../lib/dates.js'
...
// `storage` persists the database between launches. It is injected rather
// than imported so the same code runs under Capacitor Filesystem on the
// phone and under plain node:fs in scripts/test-native-db.mjs.
//   storage.SQL   — an initialised sql.js module
//   storage.load()  -> Uint8Array | null   (null on first ever launch)
//   storage.save(bytes) -> Promise         (temp file + rename, debounced)
let storageRef = null

async function init(storage) {
  storageRef = storage
  const bytes = await storage.load()
  // Every write marks the database dirty; the adapter debounces the
  // actual serialisation (~300ms) so a burst of writes costs one save.
  db = openDatabase(storage.SQL, bytes, { onWrite: () => storage.markDirty() })
  storage.attach(() => db.export())
  db.pragma('foreign_keys = ON')

  runSchemaAndMigrations()
}

// Schema + migrations, split out of init() so an imported database is
// brought up to the current shape too (see replaceWith below). Every
// statement is CREATE TABLE IF NOT EXISTS or a guarded ensureColumn, so
// running this repeatedly on an existing database is a no-op.
function runSchemaAndMigrations() {
  db.exec(`
...
// from the list that used to be hardcoded, so nothing already budgeted or
// spent under those names goes missing from the picker. Also picks up any
// category already sitting in transactions/budgets/recurring that isn't
// in that default list (hand-edited data, or an imported database).
...
// Same algorithm and same lowercase-hex encoding as the desktop app's
// node:crypto sha256, so a PIN set on Windows verifies here and an
// imported database's pin_hash keeps working. Web Crypto is async, which
// makes setPin/verifyPin async too — the only signature change in this
// file, and invisible to the renderer, whose window.api was always
// promise-based.
...
const profile = {
  ...
  setPin: async (pin) => { db.prepare('UPDATE profile SET pin_hash=? WHERE id=1').run(await hashPin(pin)); return true },
  ...
  verifyPin: async (pin) => {
    ...
    return row.pin_hash === await hashPin(pin)
  },
...
// The database as a byte array — the exact contents of a lifeos.db file.
// On the desktop this needed SQLite's online backup API because WAL mode
// keeps recent writes in a sidecar file; sql.js has no WAL and no
// sidecars, so a plain export IS the consistent snapshot. Used by
// Settings -> Backup & transfer (export) and by the debug tests.
function exportBytes() {
  return db.export()
}

// Replaces the live database wholesale — the import half of Backup &
// transfer. The caller is responsible for having validated the bytes
// (see bridge.js) and for reloading every store afterwards. Migrations
// run against the incoming file so a database from an older build of the
// desktop app is brought up to date on arrival.
async function replaceWith(bytes) {
  if (db) db.close()
  db = openDatabase(storageRef.SQL, bytes, { onWrite: () => storageRef.markDirty() })
  storageRef.attach(() => db.export())
  db.pragma('foreign_keys = ON')
  runSchemaAndMigrations()
  await storageRef.saveNow(db.export())
}

// Writes any pending changes out immediately, instead of waiting for the
// debounce. Called when Android backgrounds the app — the process can be
// killed at any point after that without warning.
async function flush() {
  if (!db || !storageRef) return
  await storageRef.flush(() => db.export())
}
...
// Direct port of the same block in electron/db.cjs — see there for the
// rationale on which tables are (and aren't) exposed here.
const DATA_TABLES = {
...
export {
  init, close, flush, exportBytes, replaceWith,
  tasks, subtasks, events, finance, accounts, savings, debts, habits, notes,
  attachments, profile, phone, data, PHONE_ALERT_MAX_AGE_MS,
}
export const raw = () => db
