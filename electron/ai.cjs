// electron/ai.cjs — thin wrapper around the Anthropic Messages API for
// the Finance page's AI features (Smart Add, Insights). Runs only in the
// main process: the API key is read from the local database and used
// only in the outbound request headers here — it's never sent to, or
...
module.exports = { parseTransaction, generateInsights }
